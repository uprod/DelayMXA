                                          ...
                                          float sliderPos, float a0, float a1, juce::Slider& s)
{
    ...
    if (isSwitch)
    {
        // Commutateur rotatif PING-PONG : 2 crans etiquetes OFF / ON.
        const float r = juce::jmin (bounds.getWidth(), bounds.getHeight()) * 0.5f - 14.0f;
        const int   numPos   = 2;
        const int   selected = juce::roundToInt (sliderPos * (float) (numPos - 1));
        static const char* names[numPos] = { "OFF", "ON" };
        ...
        for (int i = 0; i < numPos; ++i)
        {
            ...
            const auto tp = juce::Point<float> (cx, cy) + dir * (r + 14.0f);
            g.setFont (i == selected ? fonts::monoBold (12.0f) : fonts::mono (11.0f));
            g.drawText (names[i],
                        juce::Rectangle<float> (30.0f, 12.0f).withCentre (tp),
